...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        ...
        LocalDateTime time = message.getDateTime();

        try {
            Connection connection = source.getConnection();
            String query = "UPDATE messages " + "SET id = ?, author_id = ?, room_id = ?, text = ?, time = ? "
                    + "WHERE messages.id = ?;";
            PreparedStatement prepQuery = connection.prepareStatement(query);
            prepQuery.setLong(1, message.getId());
            prepQuery.setLong(2, message.getAuthor().getId());
            prepQuery.setLong(3, message.getRoom().getId());
            prepQuery.setString(4, message.getText());
            prepQuery.setTimestamp(5, Timestamp.valueOf(message.getDateTime()));
            prepQuery.setLong(6, message.getId());
            prepQuery.executeUpdate();

            prepQuery.close();
            connection.close();
        } catch (SQLException e) {
            throw new NotSavedSubEntityException("Failed to update");
        }
    }
}
